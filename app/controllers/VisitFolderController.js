...
const pdfService = require("../services/visitFolderPdf.service");
...
const _generatePdfFile = async (folderId) => {
  ...
  let property = await db.property.findById(folder.propertyId).populate("equipment outside environment serviceAccessibility leisure cooking heatingType energymode").lean();
  ...
  if (folder.editableContent?.valorizationItems?.length > 0) {
    const items = await db.valorizationItem
      ...
      .lean();
    snapshot.valorizationItems = items.map((i) => ({ label: i.label }));
  }
  ...
  const html = pdfService.buildHtml(snapshot, folder.destination, folder.selectedPhotos, property);
  await pdfService.generatePdf(html, pdfPath);
};

const SELLER_FILE_CATEGORIES = [
  { key: "carrezLaw", label: "Loi Carrez" },
  { key: "technicalDiagnostic", label: "Diagnostic technique" },
  { key: "coOwnership", label: "Règlement de copropriété" },
  { key: "personalContribution", label: "Apport personnel" },
  { key: "condominiumBooklet", label: "Carnet d'entretien" },
  { key: "minutesOfGeneral", label: "PV d'assemblée générale" },
...
const generate = async (req, res) => {
  try {
    ...
    const { propertyId, destination, selectedPhotoFileNames, visitHighlights, neighborhood, valorizationItemIds, selectedDocumentIds } = req.body;
...
const update = async (req, res) => {
  try {
    ...
    const { visitHighlights, neighborhood, valorizationItemIds, selectedPhotoFileNames, selectedDocumentIds } = req.body;
